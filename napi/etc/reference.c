...
const char *SF_FILENAME = "example.sf2";
unsigned int SF_BANK = 0, SF_PROG = 3;
...
int main(int argc, char **argv)
{
     ...
     */
    sfont_id = fluid_synth_sfload(synth, SF_FILENAME, 0);
    if(sfont_id == FLUID_FAILED)
    {
        puts("Loading the SoundFont failed!");
        goto err;
    }
    /* Select the instrument for channel 0 */
    int rc = fluid_synth_program_select(synth, 0, sfont_id, SF_BANK, SF_PROG);
    if(rc != FLUID_OK)
    {
        puts("Failed to select instrument from SoundFont!");
        goto err;
    }
